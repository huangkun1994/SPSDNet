...
def compute_self_distillation_loss(
    ...
    dcg_generator: nn.Module | None = None,
    ...
    if encoder_logits and encoder_kl_weight > 0:
        ...
        for i, enc in enumerate(student_list):
            ...
            # [B] External Distillation (DCG)
            if is_last_encoder and dcg_generator is not None:
                # Prepare aligned T0 and T1 for DCG
                t0_aligned = _resize_like(teacher_enc_T0, main_logits)
                t1_aligned = _resize_like(teacher_dec_T1, main_logits)
                mask_dcg = dcg_generator(t1_aligned, t0_aligned)

                if mask_dcg is not None:
                    mask_ratios.append(mask_dcg.mean())
                    if use_sigmoid:
                        # Use Binary JS for DCG Loss
                        dcg_loss_val = binary_js_divergence(student_probs, probs_T1)
                        if torch.isnan(dcg_loss_val).any():
                             dcg_loss_val = torch.nan_to_num(dcg_loss_val, nan=0.0)
                        
                        loss_external = (dcg_loss_val * mask_dcg).sum() / (mask_dcg.sum() + 1e-6)
                    else:
                        with torch.autocast('cuda', enabled=False):
                            # Use JS Divergence for DCG
                            student_probs_f32 = student_probs.float()
                        ...
                        loss_external = (js_val * mask_dcg).sum() / (mask_dcg.sum() + 1e-6)
                    current_layer_loss = current_layer_loss + dcg_loss_weight * loss_external
    ...
    if mask_ratios:
        components["dcg_mask_ratio"] = torch.stack(mask_ratios).mean()
    else:
        components["dcg_mask_ratio"] = torch.tensor(0.0, device=device)
